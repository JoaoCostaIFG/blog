...
export function getPostById(id: string): PostData | null {
	...
	if (!fs.existsSync(fullPath)) {
		console.warn(
			`[getPostById] File not found for post id "${id}": ${fullPath}. Skipping.`,
		);
		return null;
	}
	...
	try {
		...
		console.error(
			`[getPostById] Error reading file ${fullPath} for post id "${id}": ${(error as Error).message}`,
		);
		return null;
	}
	...
	try {
		...
		if (typeof rawData.title !== "string" || !rawData.title.trim()) {
			console.warn(
				`[getPostById] Post with id "${id}" is missing a valid title. Skipping.`,
			);
			return null;
		}
		...
		if (rawData.date === undefined || rawData.date === null) {
			console.warn(
				`[getPostById] Post with id "${id}" is missing a date. Skipping.`,
			);
			return null;
		}
		...
		if (Number.isNaN(postDate.getTime())) {
			console.warn(
				`[getPostById] Post with id "${id}" has an invalid date: "${String(rawData.date)}". Skipping.`,
			);
			return null;
		}
		...
		console.error(
			`[getPostById] Error processing markdown or frontmatter for post "${id}": ${(error as Error).message}`,
		);
		return null;
	}
}
...
export function getSortedPostsData(): PostData[] {
	if (!fs.existsSync(postsDirectory)) {
		console.error(
			`[getSortedPostsData] Posts directory not found: ${postsDirectory}`,
		);
		return [];
	}
	...
	try {
		...
		console.error(
			`[getSortedPostsData] Error reading posts directory ${postsDirectory}: ${(error as Error).message}`,
		);
		return [];
	}

	const mdFileNames = fileNamesInDir.filter((fileName) =>
	...
	if (mdFileNames.length === 0) {
		if (fileNamesInDir.length > 0) {
			console.warn(
				`[getSortedPostsData] No markdown files (.md) found in ${postsDirectory}. Files found: ${fileNamesInDir.join(", ")}`,
			);
		} else {
